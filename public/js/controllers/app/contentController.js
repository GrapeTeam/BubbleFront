'use strict';
bubbleFrame.register('contentController', function ($scope, bubble, $modal, $http, $rootScope, $timeout, $state) {
    ...
    $scope.setTop = function (v, type) {
        loadingbox.fadeIn(200);
        bubble._call(type ? "content.SetTop" : "content.cancelTop", v._id).success(function (v) {
            if (!v.errorcode) {
                swal(type ? "文章已置顶" : "文章已取消置顶");
            } else {
                swal(type ? "置顶失败" : "取消置顶失败");
            }
            $scope.pager.getData();
        });
    }
    ...
    KindEditor.plugin('hello', function (K) {
        ...
        // 点击图标时执行
        editor.clickToolbar(name, function () {
            bubble.customModal("videoAddModal.html", "videoAddController", "lg", {}, function (v) {
                var type = "video/" + v.split(".").pop();
                var tpl = '<p id="videoplugins">[' + v + ']</p>';
                editor.appendHtml(tpl);
            });
        });
    });
    ...
    var editor = KindEditor.create('#editor_id', {
        items: [
            ...
            'table', 'hr', 'baidumap', 'link', 'unlink', "hello"
        ],
        filterMode: false,
    ...
    $scope.save = function (check, p) {
        ...
        var cb = function () {
            ...
            if ($scope.mode == "new") {
                ...
                bubble._call("content.update", p._id, bubble.replaceBase64(JSON.stringify(p))).success(function (v) {
                    if (v.errorcode) {
                        swal("发布失败");
                    } else {
                        continueAppend || swal("发布成功");
                        continueAppend || ($scope.newItem = null);
                        continueAppend && continueAppend(p._id, p, true);
                        $scope.newItem = null;
                        bubble.updateScope($scope);
                    }
                    $(".btn-save").html("发布");
    ...
    var Preview = function () {
        ...
        this.show = function (item) {
            // box.fadeIn(200);
            // this.initHtml(v);
            bubble.toggleModalBtnLoading(item, true);
            $(".contentbatchMask").fadeIn(200);
            var key = '';
            var num = 18000;
            var subnum = 0;
            ...
            var rs = function (o, k) {
                ...
                key = k;
                bubble._call("content.yulan", item.ogid, $scope.mode, bubble.replaceBase64(JSON.stringify(o)), key).success(function (v) {
                    if (!v.errorcode && v) {
                        o.content += content;
                        if (count == chunks) {
                            $(".loading-spinner p").remove();
                            $(".contentbatchMask").fadeOut(200);
                            bubble.toggleModalBtnLoading(item, false);
                            window.open(v.host + v.key)
                        } else {
                            rs(o, v.key);
                        }
                        $(".loading-spinner p").html("<p>已完成" + (((count / chunks) * 100).toFixed(2)) + "%</p>");
                    } else {
                        swal('预览失败')
                    }
                });
            ...
            if (!$scope.newItem || $scope.newItem && $scope.newItem._id) {
                var id = item.ogid
                var obj = {};
                obj._id = item._id
                bubble._call('content.yulan', id, "wait", bubble.replaceBase64(JSON.stringify(obj)), key).success(function (res) {
                    if (!res.errorcode && res) {
                        $(".contentbatchMask").fadeOut(200);
                        bubble.toggleModalBtnLoading(item, false);
                        window.open(res.host + res.key)
                    } else {
                        swal('预览失败')
                    }
                })
                ...
                if (item.content.length > num) {
                    while (subnum < item.content.length) {
                        var c = item
                        list.push(c.content.substring(subnum, subnum + num));
                        subnum += num;
                    }
                    chunks = list.length;
                    item.content = list.shift();
                    rs(item);

                } else {
                    bubble._call("content.yulan", item.ogid, "new", bubble.replaceBase64(JSON.stringify(item)), key).success(function (v) {
                        if (!v.errorcode && v) {
                            $(".contentbatchMask").fadeOut(200);
                            bubble.toggleModalBtnLoading(item, false);
                            window.open(v.host + v.key)
        ...
        this.initHtml = function (v) {
            ...
            box.find(".content-info").html("").append('<span> 来源: ' + d.souce + ' </span><span> 发布时间：' + new Date(isNaN(d.time) ? d.time : parseInt(d.time)).Format("yyyy-MM-dd hh:mm:ss") + ' </span><span> 作者：' + d.author + ' </span>');
            if (d.type == 10) {
                var tmp = $("<div></div>");
    ...
    var PushConfig = function () {
        ...
        var send = function () {
            ...
            a.time = typeof a.time === "string" ? Date.parse(a.time) : a.time;
            delete a.$$hashKey;
            ...
            a.readCount = a.readCount ? parseInt(a.readCount) : a.readCount;
    ...
    var ThirdParty = function () {
        ...
        this.save = function () {
            ...
            loadingbox.fadeIn(200);
            bubble._call("wechatUser.savetowechat", current._id, getSymbolData()).success(function (v) {
                loadingbox.fadeOut(200);
                swal(v.message ? v.message : v.data);
            });
        }
...
bubbleFrame.register("pushToWechatController", function (bubble, items, $scope, $modalInstance, $timeout) {
    ...
    $scope.ok = function (e) {
        ...
        if (s) {
            bubble._call("wechatUser.pushtowechat|17", bubble.getAppId(), s._id, items).success(function (v) {
...
bubbleFrame.register("videoAddController", function (bubble, items, $scope, $modalInstance, $timeout) {
    var uploader = function () {
        var uploader = "";
        var _this = this;
        var btn = $('#video-modal-uploader');
        ...
        this.init = function (s, p, k) {
            uploader = new WebUploader.Uploader({
                ...
                swf: './js/modules/webuploader/Uploader.swf',
                server: bubble.getInterface("upload.file") + "&folderid=0&wbid=",
                pick: '#video-modal-uploader',
            });
            uploader.on("fileQueued", this.fileQueued);
            uploader.on("uploadSuccess", this.uploadSuccess);
            uploader.on("uploadProgress", this.uploadProcess);
            return this;
        }

        this.fileQueued = function (file) {
            btn.find(".webuploader-pick").html("文件上传中,请勿关闭此窗口");
            bubble.updateScope($scope);
        }

        this.uploadProcess = function (file, percentage) {
            $(".video-modal-process").width((percentage.toFixed(2) * 100) + "%");
        }

        this.uploadSuccess = function (file, v) {
            $(".video-modal-process").width(0);
            btn.find(".webuploader-pick").html("选择视频");
            !v.errorcode ? $scope.video = bubble.getInterface("upload").visible + v.filepath.replace(/\\/g, "/") : swal("上传失败");
            bubble.updateScope($scope);
        }
    ...
    $timeout(function () {
        $scope.upload = new uploader().init();
    });
