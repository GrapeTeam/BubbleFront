...
bubbleFrame.register("fileFolderCreateController", function (bubble, items, $scope, $modalInstance) {
    ...
    $scope.ok = function (e) {
        ...
        bubble._call("file.add", { fatherid: items.fid || items.fid == 0 ? items.fid : items.fid, fileoldname: $scope.name, fileextname: "folder" ,wbid:window.localStorage.siteid}).success(function (v) {
            if (v.errorcode) {
                swal(v.message);
...
bubbleFrame.register("fileMoveController", function (bubble, items, $scope, $modalInstance) {
    ...
    for(var i=0;i<$scope.data.length;i++){
        $scope.data[i].label = $scope.data[i].fileoldname
    }
