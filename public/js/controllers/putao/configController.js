bubbleFrame.register('configController', function ($scope, bubble, $modal, $http) {
    ...
    $scope.createType = function () {
        bubble.customModal("configCreate.html", "configCreateContorller", "lg", { scope: $scope, configfield: configfield, currentType: currentType, current: current }, function (v) {
    ...
    var getGroup = function (v, target, groupBy, typeid) {
        ...
        !isexist && target.push({ list: [v], name: name, visibleItem: [{ key: "configname", ch: "配置名称" }, { key: "desp", ch: "配置描述" }, { key: "id", ch: "" }], field: groupBy ? (configfield[typeid][name] ? configfield[typeid][name] : "") : (configfield[typeid] ? configfield[typeid] : "") });
                ...
                if (v.length) {
                    for (var i = 0; i < v.length; i++) {
                        try {
                            ...
                            !isexist && $scope.type.push({ list: getGroup(v[i], [], groupBy, id), id: id, name: typename, groupBy: groupBy });
...
bubbleFrame.register('configCreateContorller', function ($scope, $modalInstance, items, bubble) {
    ...
    $scope.typechange = function () {
        $scope.group = $scope.typename.list && $scope.typename.list.length ? $scope.typename.list[0] : { name: "", list: [], field: {} };
        $scope.field = $scope.group.field;
