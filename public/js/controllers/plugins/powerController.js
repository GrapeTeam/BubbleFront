bubbleFrame.register('powerController', function ($scope, bubble, $modal, $http, $rootScope, $timeout) {
    ...
    $scope.reasonList = { "1": "群众来信", "2": "群众举报", "3": "政策改革" };
    ...
    var Preview = function () {
        ...
        this.showById = function (id, fn) {
            ...
            bubble._call("content.find", id).success(function (v) {
                ...
                box.click(function (e) { if (e.target === e.currentTarget) { _this.hide(); } });
                loadingbox.fadeOut(200);
                _this.initHtml(v);
                fn && fn();
            })
    ...
    var Mind = function () {
        ...
        var getJson = function (v, fn) {
            $(".mind-wrapper .col-md-8 .mindmask").hide();
            var count = 0;
            ...
            if (v.itemChildrenData) {
                for (var i = 0; i < v.itemChildrenData.length; i++) {
                    (function (n) {
                        ...
                        jylist.children.unshift(o);
                        bubble._call("powerContent.pageBy", 1, 10, [{ field: "fatherid", logic: "=", value: o.data._id }]).success(function (rs) {
        ...
        this.siteTreeSelect = function (v) {
            ...
            bubble._call("powerType.pageBy", 1, 1000, [{ field: "wbid", logic: "=", value: v._id }]).success(function (rs) {
        ...
        var getPowerList = function (id) {
            $(".powerloading").fadeIn(200);
            bubble._call("power.pageBy", 1, 1000, [{ field: "wbid", logic: "=", value: _this.currentSite._id }, { field: "type", logic: "=", value: id }]).success(function (rs) {
        ...
        this.addMeetingTimeLine = function () {
            if (_this.powerTypeList.length)
                bubble.customModal("powerTimeLineAddModal.html", "powerTimeLineAddController", "lg", { scope: $scope }, function (v) {
        ...
        this.editMeetingTimeLine = function (v, e) {
            e.stopPropagation();
            bubble.customModal("powerTimeLineAddModal.html", "powerTimeLineAddController", "lg", { scope: $scope, value: v, edit: true }, function (v) {
                _this.siteTreeSelect(_this.currentSite);
            });
        }

        this.getHistoryTime = function (v) {
            return new Date(v).Format("yyyy.MM.dd");
        }

        //会议切换
        this.onMeetingSelect = function (v, e) {
            if (v.active && !e && _this.currentMeeting) {
                return;
            }
            ...
            $(".powerloading").fadeIn(200);
            bubble._call("power.getHistory", v._id).success(function (v) {
                _this.currentHistory = v;
                $(".powerloading").fadeOut(200);
            });
        }

        this.historyClick = function (v) {
            $(".powerloading").fadeIn(200);
            _this.currentHistoryItem = v;
        ...
        var UpdateModal = function () {
            ...
            var update = function () {
                ...
                if (node.type == 3 && node.editReason != _this.currentNode.data.editReason) {
                    p.editReason = isNaN(parseInt(node.editReason)) ? 0 : parseInt(node.editReason);
                }
                if (Object.getOwnPropertyNames(p).length) {
                    bubble._call("powerContent.update", node._id, bubble.replaceBase64(JSON.stringify(p))).success(function (v) {
                        this.hide();
                        _this.historyClick(_this.currentHistoryItem);
                    }.bind(this));
                } else {
                    minder.refresh();
                    this.hide();
        ...
        var AddModal = function () {
            ...
            this.confirm = function () {
                ...
                var p = {
                    ...
                    wbid: _this.currentSite._id,
                    fatherid: _this.currentMeeting._id,
                };

                if (this.data[2].editReason) {
                    p.editReason = parseInt(this.data[2].editReason);
                }
                ...
                if (this.data[0].name) {
                    ...
                    if (this.data[1].name) {
                        if (this.data[1].aid) {
                            p.children.children = { linkId: this.data[1].aid, type: 1, runCycle: parseInt(this.data[1].runCycle), effectiveTime: parseInt(this.data[1].runCycle) };
                        } else {
                            p.children.children = { existid: this.data[1].aid, type: 1, runCycle: parseInt(this.data[1].runCycle), effectiveTime: parseInt(this.data[1].runCycle) };
                        }
                    }
                }
                $(".powerloading").fadeIn(200);
        ...
        var PowerTypeModal = function () {
            ...
            var doAdd = function (name) {
                bubble._call("powerType.add", bubble.replaceBase64(JSON.stringify({ name: name, wbid: _this.currentSite._id }))).success(function (v) {
                    if (--queueDone == 0) {
                        this.hide();
                        _this.siteTreeSelect(_this.currentSite);
                    }
...
bubbleFrame.register("powerTimeLineAddController", function (items, $scope, $modalInstance, bubble) {
    $scope.name = items.edit ? items.value.name : "";
    $scope.type = items.scope.Mind.currentType._id;
    $scope.typelist = items.scope.Mind.powerTypeList;

    $scope.ok = function (e) {
        bubble.toggleModalBtnLoading(e, true);
        if (items.edit) {
            bubble._call("power.update", items.value._id, bubble.replaceBase64(JSON.stringify({ name: $scope.name, wbid: items.scope.Mind.currentSite._id, type: $scope.type }))).success(function (v) {
                $modalInstance.close(v);
            });
        } else {
            bubble._call("power.insert", bubble.replaceBase64(JSON.stringify({ name: $scope.name, wbid: items.scope.Mind.currentSite._id, type: $scope.type }))).success(function (v) {
                $modalInstance.close(v);
            });
        }
    }
